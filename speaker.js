...
fetch("./data.json").then((res) => {
  res.json().then((data) => {
    ...
    speakers.forEach((item, i) => {
      ...
      speakersPRoductWrapper.innerHTML += `
      <section class="hp-prduts-wrapper">
        <div class="hp-xx99-mark-ii ${item.slug}">
          ...
          <div class="hp-xx99-mark-ii-text   ${item.slug}-text">
            ...
            <a href="./product-${item.slug}.html?testing" class="prdut-btn">see product</a>
          </div>
        </div>
      </section>
          `;
      console.log(`./product-${item.slug}.html`);
    });
  });
});
...
function totalPriceInCart() {
  ...
  let newTotal = totalPrice.toLocaleString()
